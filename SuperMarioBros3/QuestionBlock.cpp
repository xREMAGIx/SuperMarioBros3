...
CQuestionBlock::CQuestionBlock(int _id_item) {
	switch (_id_item)
	{
	...
	default:
		...
		LPANIMATION_SET ani_set = animation_sets->Get(208);
...
void CQuestionBlock::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	if (state == QUESTION_BLOCK_STATE_OPENED) {
		...
		{
			case OBJECT_TYPE_MUSHROOM:
				item->SetState(MUSHSHROOM_STATE_WALKING);
				break;
			default:
				if (dynamic_cast<CCoin*>(item)->GetState() == COIN_STATE_IDLE) {
